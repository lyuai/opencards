      ...
      <section className="gameArea" aria-label="Guandan card table">
        ...
        <aside className="copilot" aria-label="AI Copilot">
          ...
          {copilotTab === "coach" ? <div className="coachPanel">
            ...
            <form className="copilotComposer" onSubmit={(event)=>{event.preventDefault(); void sendCopilotMessage();}}><textarea aria-label="Ask Arena Copilot" rows={2} value={chatDraft} onChange={(event)=>setChatDraft(event.target.value)} placeholder="Ask about this position…"/><button className="primary" disabled={!chatDraft.trim()||chatLoading}>Send</button></form>
...
function CardButton({ card, wild, recommended, selected, onClick }: { card: Card; wild: boolean; recommended: boolean; selected: boolean; onClick: () => void }) { const red=card.suit==="♥"||card.suit==="♦", joker=card.suit==="★", redJoker=joker&&card.rank==="RJ"; return <button type="button" aria-pressed={selected} aria-label={`${card.rank} ${card.suit}${wild ? " wild card" : ""}`} className={`playingCard ${red?"red":""} ${joker?"joker":""} ${redJoker?"redJoker":""} ${wild?"wild":""} ${recommended?"recommended":""}`} onClick={onClick}><span>{joker?"JOKER":card.rank}</span><i>{joker?(redJoker?"★":"☆"):card.suit}</i>{wild?<small>WILD</small>:joker&&<small>{redJoker?"RED":"BLACK"}</small>}</button>; }
function MiniCard({ card }: { card: Card }) { return <span className={card.suit==="♥"||card.suit==="♦"||card.rank==="RJ"?"red":""}>{card.suit==="★"?`${card.rank}★`:`${card.rank}${card.suit}`}</span>; }
