...
const seats = [
  { id: "north", label: "队友", role: "AI 队友", className: "north" }, { id: "west", label: "西家", role: "AI 对手", className: "west" },
  { id: "east", label: "东家", role: "AI 对手", className: "east" }, { id: "south", label: "你", role: "玩家", className: "south" },
] as const;
  ...
  return <main>
    ...
    <div className="playWorkspace">
      <section className="gameArea" aria-label="Guandan card table">
        ...
        <div className="turnOrder" aria-label="出牌顺序"><span className={game?.turn === "south" ? "current" : ""}>你（南）</span><i>→</i><span className={game?.turn === "east" ? "current" : ""}>东家</span><i>→</i><span className={game?.turn === "north" ? "current" : ""}>队友（北）</span><i>→</i><span className={game?.turn === "west" ? "current" : ""}>西家</span></div>
        <div className="table">
        ...
        <div className="handLabel"><b>你的手牌</b><span>{game?.yourHand.length ?? 0} 张 · 已选 {selected.length} 张</span></div>
        <div className="hand" aria-label="你的手牌">{game?.yourHand.map((card) => <CardButton card={card} wild={card.rank === game.levelRank && card.suit === "♥"} recommended={aiAdvice?.cardIds.includes(card.id) ?? false} selected={selected.includes(card.id)} key={card.id} onClick={() => yourTurn && setSelected((current) => current.includes(card.id) ? current.filter((id) => id !== card.id) : [...current, card.id])}/>)}</div>
        <div className="playControls">
          <button className="secondary" disabled={!yourTurn || loading || !game?.currentPlay} onClick={() => void act(true)}>不出</button>
          <button className="primary" disabled={!yourTurn || loading || selected.length === 0} onClick={() => void act(false)}>{loading ? "出牌中…" : "出所选牌"}</button>
        </div>
