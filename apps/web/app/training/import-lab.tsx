...
type ImportJob = { id: string; status: string; stage: string; progress: number; message?: string; result?: { extractionStatus?: string; message?: string; observations?: unknown[] } };
...
export function ImportLab() {
  ...
  return <main>
    ...
    <section className="importPanel" aria-label="Video import">
      ...
      {job?.result && <div className="captureSummary"><b>{job.result.observations?.length ?? 0} changed frames retained</b><span>{job.result.message}</span></div>}
      {error && <p className="error">{error}. Is the Go API running?</p>}
    </section>
    <section className="captureSteps"><b>Browser capture</b><ol><li>Load the unpacked extension from <code>extensions/browser</code>.</li><li>Open the matching Bilibili video and start playback.</li><li>Click OpenCards Capture to start; click it again at the end.</li></ol><p>Frames stay local. Static frames are rejected before OCR or AI processing.</p></section>
  </main>;
}
